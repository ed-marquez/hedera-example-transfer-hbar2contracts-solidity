...
async function main() {
	...
	const [tokenTxStatus, tokenId] = await tokenCreatorFcn(treasuryId, treasuryKey);
...
====================================================`);
	...
	let moveAmt = 30;
	const transferSdkRx = await hbar2ContractSdkFcn(treasuryId, contractId.toString(), moveAmt, treasuryKey);
...
async function tokenCreatorFcn(treasuryId, treasuryKey) {
	...
	return [tokenCreateReceipt.status, tokenCreateReceipt.tokenId];
}
...
async function contractExecuteNoFcn(cId, gasLim, amountHbar) {
	const contractExecuteTx = new ContractExecuteTransaction().setContractId(cId).setGas(gasLim).setPayableAmount(amountHbar);
...
async function contractCallQueryFcn(cId, gasLim, fcnName) {
	const contractQueryTx = new ContractCallQuery().setContractId(cId).setGas(gasLim).setFunction(fcnName);
...
async function contractParamsBuilderFcn(aId, amountHbar, section, tId) {
	let builtParams = [];
	if (section === 2) {
		builtParams = new ContractFunctionParameters().addAddress(aId.toSolidityAddress()).addAddress(tId.toSolidityAddress());
	} else if (section === 3) {
		builtParams = new ContractFunctionParameters().addAddress(aId.toSolidityAddress()).addUint256(amountHbar * 1e8);
	} else {
	}
	return builtParams;
}
